...
with Timer():
    fDenoised = pd_hybrid_grad_alg(
        f,
        lambda u, tau: proxL2Data(u, tau, f),
        project_unit_ball2D,
        gradient_FD2D,
        divergence_FD2D,
        lambda_,
...
plt.subplot(1, 2, 1)
plt.imshow(f, interpolation="nearest", cmap="gray", vmin=0, vmax=1)
plt.axis("off")
plt.title("Input image")

plt.subplot(1, 2, 2)
plt.imshow(fDenoised, interpolation="nearest", cmap="gray", vmin=0, vmax=1)
plt.axis("off")
