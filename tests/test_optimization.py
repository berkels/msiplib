...
def hess_f(x, params):
    ...
    tmp = f(x, params)
    hess[:, 0, 0] = tmp * 2 / params[1] ** 2 * (2 * x**2 - 4 * x * params[0] + 2 * params[0] ** 2 - params[1])
    hess[:, 0, 1] = (
        tmp * 2 / params[1] ** 3 * (x - params[0]) * (x**2 - 2 * x * params[0] + params[0] ** 2 - params[1])
    )
    hess[:, 0, 2] = tmp * 2 / params[1] * (x - params[0]) / params[2]
    hess[:, 1, 0] = hess[:, 0, 1]
    hess[:, 1, 1] = (
        tmp * (x - params[0]) ** 2 * (x**2 - 2 * x * params[0] + params[0] ** 2 - 2 * params[1]) / params[1] ** 4
    )
...
class TestOptimizationMethods(unittest.TestCase):
    def test_gradient_descent(self):
        ...
        params_GD = GradientDescent(
            params_initial,
            lambda params: E_fit(sampling_nodes, sampling_values, params),
            lambda params: E_fit_grad(sampling_nodes, sampling_values, params),
            maxIter=1000,
        )
